...
const LogGoalModal = ({ match, onClose, players, onSave }) => {
  const [goalData, setGoalData] = useState({ scorer_id: '', assist_id: '', minute: '', team_id: '' });
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-card w-full max-w-md rounded-xl p-6 border border-gray-700">
        <h3 className="text-white font-bold text-lg mb-4">Log Goal: {match.home_team?.name} vs {match.away_team?.name}</h3>
        
        <select className="w-full bg-dark p-2 mb-3 rounded" onChange={(e) => setGoalData({...goalData, team_id: e.target.value})}>
        ...
        <select className="w-full bg-dark p-2 mb-3 rounded" onChange={(e) => setGoalData({...goalData, scorer_id: e.target.value})}>
          <option value="">Select Scorer</option>
          {relevantPlayers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>

        <select className="w-full bg-dark p-2 mb-3 rounded" onChange={(e) => setGoalData({...goalData, assist_id: e.target.value})}>
          <option value="">Select Assist (Optional)</option>
          {relevantPlayers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>

        <input type="number" placeholder="Minute" className="w-full bg-dark p-2 mb-4 rounded" onChange={(e) => setGoalData({...goalData, minute: e.target.value})} />
        
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 bg-gray-600 p-2 rounded">Cancel</button>
          <button onClick={() => onSave(goalData)} className="flex-1 bg-brand p-2 rounded text-dark font-bold">Save Goal</button>
        </div>
      </div>
    </div>
  );
...
export default function Matches() {
  ...
  const fetchData = async () => {
    ...
    const goalsRes = await fetch('http://localhost:8000/goals/'); // NEW FETCH
  ...
  const handleUpdateMatch = async (id, field, value) => {
    console.log(`DEBUG: Updating match ${id} | Field: ${field} | Value: ${value}`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatusMessage(`Update Failed: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
...
const handleDeleteGoal = async (goalId) => {
  ...
  if (!window.confirm("Disallow this goal? The match score will automatically decrement.")) return;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10">
      ...
      <div>
        ...
        <p className="text-gray-400 mt-1">Record new matches and manage live scores.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-dark p-6 rounded-lg border border-gray-800">
            ...
            <p className="text-gray-500 text-xs text-center mt-2">
              Leave blank if the match is already played. Set a future date to schedule a fixture.
            </p>
          </div>
...
const GoalForm = ({ match, players, onSave }) => {
  const [data, setData] = useState({ scorer_id: '', assist_id: '', minute: '', team_id: '' });

  return (
    <div className="space-y-4">
      <h3 className="text-brand font-bold">Log Event</h3>
      <select className="w-full bg-dark p-2 rounded text-white" onChange={(e) => setData({...data, team_id: e.target.value})}>
      ...
      <select className="w-full bg-dark p-2 rounded text-white" onChange={(e) => setData({...data, scorer_id: e.target.value})}>
        <option value="">Scorer</option>
        {players.filter(p => p.team_id == data.team_id).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
      </select>

      <select className="w-full bg-dark p-2 rounded text-white" onChange={(e) => setData({...data, assist_id: e.target.value})}>
        <option value="">Assist (Optional)</option>
        {players.filter(p => p.team_id == data.team_id).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
      </select>

      <input type="number" placeholder="Minute" className="w-full bg-dark p-2 rounded text-white" onChange={(e) => setData({...data, minute: e.target.value})} />
      
      <button onClick={() => onSave({...data, match_id: match.id})} className="w-full bg-brand p-3 rounded font-bold text-dark">Log Goal</button>
    </div>
  );
};
...
const MatchManagementRow = ({ 
  match, teams, players, goals, getTeamName, 
  handleUpdateMatch, handleLogGoal, handleUpdateGoal, handleDeleteGoal 
}) => {
  const [inlineGoal, setInlineGoal] = useState({ scorer_id: '', assist_id: '', minute: '', team_id: '' });
  
  // State to track which goal ID is currently being modified inline
  const [editingGoalId, setEditingGoalId] = useState(null);
  const [editForm, setEditForm] = useState({ scorer_id: '', assist_id: '', minute: '' });

  const safePlayers = Array.isArray(players) ? players : [];
  const safeGoals = Array.isArray(goals) ? goals : [];

  // Filter players and match specific goals
  const matchPlayers = safePlayers.filter(p => p.team_id === match.home_team_id || p.team_id === match.away_team_id);
  const activeMatchGoals = safeGoals.filter(g => g.match_id === match.id).sort((a, b) => a.minute - b.minute);

  const getPlayerName = (id) => safePlayers.find(p => p.id === id)?.name || `Unknown Player (${id})`;

  const submitInlineGoal = () => {
    if (!inlineGoal.scorer_id || !inlineGoal.minute || !inlineGoal.team_id) {
      alert("Please select a Team, Scorer, and enter the Minute to log the goal.");
      return;
    }
    handleLogGoal({
      match_id: match.id,
    ...
    <div className="p-6 flex flex-col gap-4 hover:bg-gray-800/30 transition-colors">
      ...
      <div className="flex flex-col xl:flex-row items-center justify-between gap-6 w-full">
        ...
        <div className="flex-1 flex items-center justify-center gap-4 w-full">
          <div className="flex-1 text-right font-bold text-gray-300">{match.home_team?.name || getTeamName(match.home_team_id)}</div>
          <div className="flex items-center gap-3 bg-dark p-2.5 rounded-lg border border-gray-700 shadow-inner">
            <input type="number" min="0" value={match.home_goals} onChange={(e) => handleUpdateMatch(match.id, 'home_goals', parseInt(e.target.value) || 0)} className="w-14 bg-card border border-gray-700 rounded text-center text-xl font-bold text-white py-1"/>
            <span className="text-gray-500 font-bold">:</span>
            <input type="number" min="0" value={match.away_goals} onChange={(e) => handleUpdateMatch(match.id, 'away_goals', parseInt(e.target.value) || 0)} className="w-14 bg-card border border-gray-700 rounded text-center text-xl font-bold text-white py-1"/>
          </div>
          <div className="flex-1 text-left font-bold text-gray-300">{match.away_team?.name || getTeamName(match.away_team_id)}</div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-center bg-dark/40 p-3 rounded-lg border border-gray-800/80">
          <select value={inlineGoal.team_id} onChange={(e) => setInlineGoal({ ...inlineGoal, team_id: e.target.value, scorer_id: '', assist_id: '' })} className="bg-card text-white text-xs border border-gray-700 rounded-lg py-2 px-2 w-32">
            <option value="">Select Team</option>
            <option value={match.home_team_id}>Home Team</option>
            <option value={match.away_team_id}>Away Team</option>
          </select>

          <select value={inlineGoal.scorer_id} onChange={(e) => setInlineGoal({ ...inlineGoal, scorer_id: e.target.value })} disabled={!inlineGoal.team_id} className="bg-card text-white text-xs border border-gray-700 rounded-lg py-2 px-2 w-36 disabled:opacity-40">
            <option value="">⚽ Scorer</option>
            {matchPlayers.filter(p => p.team_id === parseInt(inlineGoal.team_id)).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>

          <select value={inlineGoal.assist_id} onChange={(e) => setInlineGoal({ ...inlineGoal, assist_id: e.target.value })} disabled={!inlineGoal.team_id} className="bg-card text-white text-xs border border-gray-700 rounded-lg py-2 px-2 w-36 disabled:opacity-40">
            <option value="">👟 Assist (Opt)</option>
            {matchPlayers.filter(p => p.team_id === parseInt(inlineGoal.team_id)).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>

          <input type="number" placeholder="Min" min="1" max="120" value={inlineGoal.minute} onChange={(e) => setInlineGoal({ ...inlineGoal, minute: e.target.value })} className="w-16 bg-card border border-gray-700 rounded-lg text-center text-xs text-white py-2" />
          <button onClick={submitInlineGoal} className="bg-brand hover:bg-emerald-400 text-dark font-bold text-xs py-2 px-3 rounded-lg transition-colors">Log Goal</button>
        </div>

        {/* Status Control */}
        <div className="w-full md:w-48 xl:w-36">
          <select value={match.status || 'Finished'} onChange={(e) => handleUpdateMatch(match.id, 'status', e.target.value)} className={`w-full text-sm font-bold border rounded-lg py-2.5 px-3 text-center cursor-pointer appearance-none transition-colors ${match.status === 'Live' ? 'bg-red-500/10 text-red-500 border-red-500/30' : match.status === 'Finished' ? 'bg-gray-800 text-gray-400 border-gray-700' : 'bg-brand/10 text-brand border-brand/30'}`}>
      ...
      {activeMatchGoals.length > 0 && (
        <div className="mt-2 bg-dark/20 border border-gray-800/60 rounded-lg p-3 flex flex-col gap-2">
          <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1">Logged Events</div>
          
          <div className="flex flex-col gap-1.5">
            {activeMatchGoals.map(goal => {
              const isEditing = editingGoalId === goal.id;
              const currentTeamPlayers = safePlayers.filter(p => p.team_id === goal.team_id);
              
              return (
                <div key={goal.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-card/60 rounded-md border border-gray-800/40 p-2 text-xs gap-3">
                  ...
                  {isEditing ? (
                    ...
                    <div className="flex flex-wrap items-center gap-2 flex-1">
                      ...
                      <select 
                        ...
                        onChange={(e) => setEditForm({ ...editForm, scorer_id: e.target.value })} 
                      ...
                      <select 
                        ...
                        onChange={(e) => setEditForm({ ...editForm, assist_id: e.target.value })} 
                        className="bg-dark text-white border border-gray-700 rounded py-1 px-1.5 max-w-[140px]"
                      >
                        <option value="">No Assist</option>
                        {currentTeamPlayers.filter(p => p.id !== parseInt(editForm.scorer_id)).map(p => (
                    ...
                    <div className="flex items-center gap-2 text-gray-300">
                      ...
                      <span className="font-semibold text-white">⚽ {getPlayerName(goal.scorer_id)}</span>
                      {goal.assist_id && (
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    {isEditing ? (
                      <>
                        <button onClick={() => saveGoalEdit(goal.id)} className="text-emerald-400 hover:bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20 font-medium">Save</button>
                        <button onClick={() => setEditingGoalId(null)} className="text-gray-400 hover:bg-gray-700 px-2 py-1 rounded border border-gray-700">Cancel</button>
                      </>
                    ) : (
                      <>
                        <button 
                          onClick={() => {
                            ...
                            setEditForm({ scorer_id: goal.scorer_id, assist_id: goal.assist_id || '', minute: goal.minute });
                          }} 
