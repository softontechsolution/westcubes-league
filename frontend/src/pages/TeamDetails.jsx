...
export default function TeamDetails() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // 2. Fetch Players
        const playersRes = await fetch(`http://localhost:8000/teams/${team_id}/players/`);
        const playersData = await playersRes.json();
        setPlayers(playersData);

        // 3. Fetch Matches
        const matchesRes = await fetch(`http://localhost:8000/teams/${team_id}/matches/`);
  ...
  if (!team) return <div className="text-white text-center py-20">Loading team profile...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-400 font-medium mt-1">Managed by {team.manager_name}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="bg-card p-6 rounded-xl border border-gray-800">
          ...
          {players.length === 0 ? <p className="text-gray-500">No players yet.</p> : (
            <table className="w-full">
              <tbody>
                {players.map(p => (
                  <tr key={p.id} className="border-b border-gray-800/50">
                    <td className="py-2 text-brand font-mono">#{p.squad_number}</td>
        ...
        <section className="bg-card p-6 rounded-xl border border-gray-800">
            ...
            {matches.length === 0 ? <p className="text-gray-500">No matches recorded.</p> : (
            <div className="space-y-3">
                ...
                return (
                    <div key={m.id} className="flex justify-between items-center bg-dark p-3 rounded-lg border border-gray-800">
                    <span className="text-white font-medium">vs {opponentName}</span>
                    <span className="text-xl font-bold text-brand">{m.home_goals} - {m.away_goals}</span>
                    </div>
                );
                })}
            </div>
