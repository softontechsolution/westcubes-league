...
const MatchInfoModal = ({ match, onClose, teams, players, goals = [] }) => {
  ...
  const homeTeamName = teams.find(t => t.id === match.home_team_id)?.name || 'Home Team';
  const awayTeamName = teams.find(t => t.id === match.away_team_id)?.name || 'Away Team';
  ...
  if (match.status === 'Finished' || match.status === 'Live') {
    timelineEvents.push({ id: 'milestone-ht', type: 'MILESTONE', minute: 45, label: 'Half Time (HT)' });
  }
  if (match.status === 'Finished') {
    timelineEvents.push({ id: 'milestone-ft', type: 'MILESTONE', minute: 90, label: 'Full Time (FT)' });
  }
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-card w-full max-w-2xl rounded-2xl border border-gray-800 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="bg-dark p-4 flex justify-between items-center border-b border-gray-800">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-white font-bold text-xl w-8 h-8 rounded-full flex items-center justify-center bg-gray-800/50 hover:bg-gray-700 transition-colors">&times;</button>
        </div>
        ...
        <div className="p-8 flex justify-between items-center bg-gradient-to-b from-gray-900/40 to-transparent border-b border-gray-800/60 relative text-center">
          <div className="text-center w-1/3 flex flex-col items-center">
            ...
            <h3 className="text-xl font-bold text-white tracking-wide">{homeTeamName}</h3>
          </div>
          ...
          <div className="text-center w-1/3 flex flex-col items-center justify-center">
            <div className="text-4xl font-mono font-black text-white bg-dark px-4 py-1.5 rounded-xl border border-gray-800 shadow-inner tracking-tight">
              {match.status === 'Scheduled' ? (
                <span className="text-gray-500 text-2xl font-medium italic tracking-widest">VS</span>
              ) : (
                <>
                  <span className={match.status === 'Live' ? 'text-red-500' : 'text-white'}>{match.home_goals}</span>
                  <span className="text-gray-600 mx-2">:</span>
                  <span className={match.status === 'Live' ? 'text-red-500' : 'text-white'}>{match.away_goals}</span>
                </>
              )}
            </div>
          ...
          <div className="text-center w-1/3 flex flex-col items-center">
            ...
            <h3 className="text-xl font-bold text-white tracking-wide">{awayTeamName}</h3>
          </div>
        </div>

        {/* LiveScore Styled Timeline Stream */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-dark/10">
          <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest px-1 mb-4">Match Events</h4>
            ...
            <div className="relative border-l-2 border-gray-800 ml-4 sm:mx-auto sm:border-l-0 sm:before:absolute sm:before:left-1/2 sm:before:top-0 sm:before:bottom-0 sm:before:w-[2px] sm:before:bg-gray-800">
              ...
              {timelineEvents.map((event) => {
                if (event.type === 'MILESTONE') {
                  return (
                    <div key={event.id} className="relative mb-6 text-center flex justify-center w-full items-center z-10">
                ...
                return (
                  <div key={event.id} className="relative flex items-start mb-6 w-full text-xs">
                    ...
                    <div className={`flex-1 pl-6 sm:pl-0 sm:pr-6 text-left sm:text-right ${isHomeEvent ? 'block' : 'hidden sm:block opacity-0 pointer-events-none'}`}>
                      {isHomeEvent && (
                        <div className="bg-card/50 border border-gray-800 rounded-lg p-2.5 inline-block max-w-[85%] shadow-sm">
                          <div className="font-bold text-white">⚽ {event.mainPlayer}</div>
                          {event.subDetail && <div className="text-gray-400 text-[11px] mt-0.5">{event.subDetail}</div>}
                        </div>
                      )}
                    </div>
                    ...
                    <div className={`flex-1 pl-6 text-left ${!isHomeEvent ? 'block' : 'hidden sm:block opacity-0 pointer-events-none'}`}>
                      {!isHomeEvent && (
                        <div className="bg-card/50 border border-gray-800 rounded-lg p-2.5 inline-block max-w-[85%] shadow-sm">
                          <div className="font-bold text-white">⚽ {event.mainPlayer}</div>
                          {event.subDetail && <div className="text-gray-400 text-[11px] mt-0.5">{event.subDetail}</div>}
...
export default function Dashboard() {
      ...
      try {
        ...
        if (matchesRes.ok && goalsRes.ok) {
          ...
          setSelectedMatch((currentSelected) => {
            ...
            return freshMatches.find(m => m.id === currentSelected.id) || currentSelected;
          });
        }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white tracking-wide">League Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-card rounded-xl border border-gray-800 shadow-md h-fit">
          <div className="bg-dark p-4 border-b border-gray-800 rounded-t-xl font-bold text-white uppercase tracking-wider">League Clubs</div>
          <div className="max-h-[600px] overflow-y-auto">
            {teams.map((team) => (
              <Link key={team.id} to={`/teams/${team.id}`} className="flex items-center gap-3 p-4 border-b border-gray-800/50 hover:bg-gray-800/30">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          {liveMatches.length > 0 && (
            <section>
              ...
              <div className="space-y-2">
                {liveMatches.map(m => (
                  <div key={m.id} onClick={() => setSelectedMatch(m)} className="bg-red-500/10 border border-red-500/20 p-4 rounded-lg flex justify-between items-center cursor-pointer hover:bg-red-500/20 transition-colors">
                    <span className="font-bold text-white w-1/3 text-right truncate">{m.home_team?.name || getTeamName(m.home_team_id)}</span>
                    <span className="text-red-500 font-mono font-black text-xl px-4 text-center">{m.home_goals} : {m.away_goals}</span>
                    <span className="font-bold text-white w-1/3 text-left truncate">{m.away_team?.name || getTeamName(m.away_team_id)}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              <h2 className="text-lg font-bold text-white uppercase tracking-wider">Match Center</h2>
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                [...matches]
                  ...
                  .map(m => {
                    const getStatusStyles = (status) => {
                      if (status === 'Live') return 'bg-red-500/5 border-red-500/20 hover:bg-red-500/10';
                      if (status === 'Finished') return 'bg-emerald-500/5 border-emerald-500/20 hover:bg-emerald-500/10';
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <div className="w-32 flex flex-col items-start shrink-0">
                            ...
                            {m.status === 'Finished' && <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">FINAL</span>}
                            {m.status === 'Scheduled' && <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">PLANNED</span>}
                          </div>
                          ...
                          <div className="flex-1 flex items-center justify-center gap-4 px-2">
                            ...
                            <div className={`flex items-center gap-2 font-black text-sm font-mono ${m.status !== 'Scheduled' ? 'bg-dark px-3 py-1 rounded border border-gray-800' : ''}`}>
                              {m.status === 'Scheduled' ? (
                                <span className="text-gray-500 text-xs font-medium tracking-widest italic">VS</span>
                              ) : (
                                <>
                                  <span className={m.status === 'Live' ? 'text-red-500' : 'text-brand'}>{m.home_goals}</span>
                                  <span className="text-gray-600">:</span>
                                  <span className={m.status === 'Live' ? 'text-red-500' : 'text-brand'}>{m.away_goals}</span>
                                </>
                              )}
                            </div>
